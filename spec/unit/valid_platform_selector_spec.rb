...
describe "a ValidPlatformSelector with prefer_binary_platform == true and a gem_platform passed" do
  ...
  it "should output a message indicating what platforms it selected" do
...
describe "a ValidPlatformSelector with prefer_binary_platform == false and no gem_platform passed" do
  before(:each) do
    common_setup_valid_platform_selector(false)
  end

  it "should select default platform (ruby) first, and @ruby_platform platform last" do
    should_select_correct_valid_platforms("i686-darwin8.7.1", nil, ['ruby', '686-darwin'])
  end
end

describe "a ValidPlatformSelector with prefer_binary_platform == false and a gem_platform passed" do
  before(:each) do
    common_setup_valid_platform_selector(false)
  end

  it "should select gem_platform first, default platform (ruby) next, and @ruby_platform platform last" do
    should_select_correct_valid_platforms("i686-darwin8.7.1", "gem-platform", ['gem-platform', 'ruby', '686-darwin'])
  end
end

describe "a ValidPlatformSelector with Gem::Platform::CURRENT platform passed" do
  before(:each) do
    common_setup_valid_platform_selector(false)
  end

  it "should select only @ruby_platform platform" do
    should_select_correct_valid_platforms("ruby_platform", Gem::Platform::CURRENT, ['ruby_platform'])
  end
end

describe "a ValidPlatformSelector with exact_platform_match = true" do
  before(:each) do
    common_setup_valid_platform_selector(false)
  end

  it "should select only gem platform platform" do
